                    ...
                    <div class="card">
                        <div class="header">
                            ...
                            <div class="pull-right">
                                <a href="{{route('reference.golongan.create')}}" class="btn btn-primary waves-effect">
                        ...
                        <div class="body">
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                                    ...
                                    <tbody>
                                        {{'',$no=1}}
                                        @foreach($golongan as $model)
                                        <tr>
                                            <td>{{$no++}}</td>
                                            <td>{{$model->nama}}</td>
                                            <td>{{$model->pangkat}}</td>
                                            <td>
                                            	<a href="{{route('reference.golongan.edit',$model->id)}}" class="btn btn-warning waves-effect">
				                                    <i class="material-icons">create</i>
				                                    <span>Edit</span>
				                                </a>

				                                <a href="{{route('reference.golongan.delete')}}" class="btn btn-danger waves-effect" onclick="event.preventDefault();deleteAlert({{$model->id}})">
				                                    <i class="material-icons">delete</i>
				                                    <span>Hapus</span>
				                                </a>

				                                <form id="form-delete-{{$model->id}}" style="display: none;" method="post" action="{{route('reference.golongan.delete')}}">
				                                	@csrf
